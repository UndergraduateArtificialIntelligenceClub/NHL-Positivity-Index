...
def label_comment_sentiment(comment: str) -> dict:
    ...
    print(comment)
    sentiment_label = input(
        "Input Sentiment Label: (p) Positive (k) Neutral (n) Negative (d) Delete Comment (q) quit "
    )
...
def label_comment_sample(sample: [str]) -> [dict]:
    ...
    for j in range(len(sample) - 1):
        ...
        print(
            f"{1000 - j} Comments left!"
        )  # If you don't want the comments left tracker just comment out
        comment = sample[j]
        ...
        if comment_label == "q":
            ...
            sample[j] = {"comment": comment, "label": comment_label}
...
def write_labelled_comments_to_json_file(
    json_file_path: str, labelled_comments: [dict]
):
    """Adds labelled comments to a json_file
    Args:
        json_file_path (str): json file path
...
def evaluate_disagreements(
    labelled_comments: List[Dict[str, str]], disagreements: List[Dict[str, str]]
) -> List[Dict[str, str]]:
    ...
    for disagreement in disagreements:
        ...
        user_input = input(
            "Would you like to change your label to the model's label? Yes (y) or No (n) "
        )
